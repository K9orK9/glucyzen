...
document.addEventListener('click',e=>{
  ...
  if(rangeBtn){
    ...
    if(TIMELINE_RANGES.includes(h)){
      timelineHours=h; localStorage.setItem(RANGE_KEY,String(h));
      syncTimelineControls(); if(latestData) renderAllTimelines(latestData);
    }
    return;
  }
...
function injectTimelineStyles(){
  ...
  link.rel='stylesheet'; link.href='/timeline-v09.css'; link.dataset.glucyzenTimelineCss='1';
  document.head.appendChild(link);
}

function installTimelineControls(){
  ['chart','glucoseDetailChart','historyChart'].forEach(id=>{
    const svg=$(id); if(!svg || svg.previousElementSibling?.classList?.contains('timeline-toolbar')) return;
...
function renderTimeline(data,target='chart'){
  ...
  const allPoints=(timeline.glucose?.length?timeline.glucose:data?.chart||[]).map(p=>({t:Number(p.t),value:Number(p.value)})).filter(p=>Number.isFinite(p.t)&&Number.isFinite(p.value)).sort((a,b)=>a.t-b.t);
  const allEvents=(timeline.events||[]).map(e=>({...e,t:Number(e.t),value:Number(e.value)})).filter(e=>Number.isFinite(e.t)).sort((a,b)=>a.t-b.t);
  const allBasal=(timeline.basal||[]).map(b=>({...b,t:Number(b.t),rate:Number(b.rate)})).filter(b=>Number.isFinite(b.t)&&Number.isFinite(b.rate)).sort((a,b)=>a.t-b.t);
  if(!allPoints.length){svg.innerHTML='<text x="50%" y="50%" text-anchor="middle" fill="#8090a7" font-size="14">Aucune donnée graphique disponible</text>';return;}
  ...
  const eventHeight=hasEvents ? (detail?58:52) : 24;
  const height=(detail?326:282)+basalHeight+(hasEvents?24:0);
  ...
  if(timelineLayers.basal && basalHeight){
    if(hasBasal){
      ...
      let seed=basal[0];
      const steps=[];
      let startX=Math.max(left,pxX(seed.t));
      steps.push(`M ${startX} ${rateY(seed.rate)}`);
  ...
  const yMarks=[40,55,70,120,180,260,400].filter(v=>v>=minY&&v<=maxY);
  yMarks.forEach(v=>{
  ...
  if(hasEvents){
    const rows=[plotBottom+22,plotBottom+45];
    let idx=0,lastX=-999,lastRow=0;
    events.forEach(e=>{
      const x=pxX(e.t),isCarbs=e.type==='carbs',icon=isCarbs?'🍞':'💉',label=e.label||`${e.value}${isCarbs?' g':' U'}`;
      if(x-lastX<70) lastRow=1-lastRow; else lastRow=idx%2;
      const row=rows[lastRow]; idx++; lastX=x;
      const bg=isCarbs?'#fff6df':'#edf6ff',stroke=isCarbs?'#e9bd60':'#6baeea',text=isCarbs?'#7f5a12':'#1f6fae';
      const w=Math.max(48,33+String(label).length*5.7),rx=Math.max(left,Math.min(width-right-w,x-w/2));
      html+=`<line x1="${x}" x2="${x}" y1="${plotBottom-2}" y2="${row-12}" stroke="${stroke}" stroke-width="1" stroke-dasharray="2 3"/>`;
      html+=`<rect x="${rx}" y="${row-15}" width="${w}" height="22" rx="11" fill="${bg}" stroke="${stroke}"/>`;
      html+=`<text x="${rx+7}" y="${row}" font-size="10" fill="${text}">${icon} ${esc(label)}<title>${new Date(e.t).toLocaleString('fr-FR',{hour:'2-digit',minute:'2-digit'})} · ${esc(label)}</title></text>`;
    });
  }
...
function renderAllTimelines(data){
  ...
  const label=`${timelineHours} h`;
  document.querySelectorAll('.timeline-current-range').forEach(el=>el.textContent=label);
}
...
function render(data){
  ...
  renderAllTimelines(data); renderDetailViews(data); window.dispatchEvent(new CustomEvent('glucyzen:data',{detail:data}));
}
